...
2. JWT 기반 인증을 통해 사용자 인증 상태를 유지한다.
3. 연동할 실내공기측정 센서를 킨다
4. 등록된 식물 정보와 탄소포집 계수를 기반으로 예상 탄소 저감량을 계산한다.
5. 계산된 탄소 포집량과 센서 기반 공기질 지수를 마이페이지에서 확인한다.
 ...
 **페이지 시나리오**
1. ADMIN관리자 계정으로 로그인한다
2. 관리자 페이지로 넘어간다(관리자 이외에는 사이트를 금한다)
3. 관리자는 관리자들 관리 회원관리 게시글관리 식물관리 대시보드 등 모든 사이트의 주요요소를 담당
4. 관리자는 자신이 원하는 대로 페이지 추가 삭제 및 복구 회원 마스킹등 모든기능을 관리하여 관리자로 끌어들이거나 회원 추가 삭제 가능
5. 또한 중요 공지는 공지사항을 통하여 등록한다 (관리자만 가능)
...
**페이지 시나리오**
1. 사용자가 로그인 하면 메인페이지로 들어가진다
2. 식물 페이지나 게시글 페이지를 들어간다
3. 식물 리스트페이지나 게시글 리스트 페이지로 들어간다
4. 식물 상세 페이지나 게시글 상세 페이지로 들어간다
5. 게시글을 쓰거나 리뷰를 작성할수있다
