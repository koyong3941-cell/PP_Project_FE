# PlantPlants

> 식물 재배 난이도와 탄소포집 성능을 공유하는 친환경 원예 커뮤니티

[데모](배포주소) · [아키텍처](#5-아키텍처-구조) · [시작 가이드](#시작-가이드)

## 목차
- [1. 팀원](#1-팀원)
- [2. 기간 및 개발 분기](#2-기간-및-개발-분기)
- [3. 왜 만들었는지?](#3-왜-만들었는지)
- [4. 이 프로젝트가 무엇을 하는건지?](#4-이-프로젝트가-무엇을-하는건지)
- [5. 아키텍처 구조](#5-아키텍처-구조)
- [6. 주요기능](#6-주요기능)

---

## 1. 팀원

**팀명** : (팀명 입력)

| 이름 | GitHub | Email | 담당 | 소개 |
|------|--------|-------|------|------|
| 성현 | [GitHub](https://github.com/koyong3941-cell) | koyong3941@gmail.com | 팀장 | 조직의 목표와 서비스의 책임을 우선적으로 고려하며, 프로젝트의 방향성에 맞는 개발을 지향합니다. |
| 일섭 | [GitHub](링크) | 이메일 | 담당 파트 | 자기소개 + 포부 |
| 민범 | [GitHub](링크) | 이메일 | 담당 파트 | 자기소개 + 포부 |
| 세웅 | [GitHub](링크) | 이메일 | 담당 파트 | 자기소개 + 포부 |


---

## 기술 스택

<!-- 예시 (실제 사용한 기술로 교체) -->
<img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=openjdk&logoColor=white">
<img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white">
<img src="https://img.shields.io/badge/Spring_Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white">
<img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white">
<img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black">
<img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white">

---

## 2. 기간 및 개발 분기

**개발 기간** : 2026.06.04 ~ 2026.07.15

### 개발 분기
- **1주차** : 친환경 탄소중립 식물 원예 커뮤니티 사이트의 방향성 및 세부 기능 구성 논의 
- **2주차** : 개발 환경 구성 및 기술 검증, 기본 CRUD 구조 테스트
- **3주차** : 기초기획, 유스케이스 작성 및 화면설계, 개발 명세 작성 시작
- **4주차** : 기획 완료, 개발 시작
- **5주차** : 기본 기능 개발 완료
- **6주차** : MVP 기능 개발 완료
- **7주차** : Admin 개발 완료
  
---

## 3. 왜 만들었는지?

평소 영화 리뷰 사이트를 자주 이용하던 팀원이 자기가 보유한 식물도 리뷰하고 재배 난이도 및 생존력 등 정보를 공유해서 타인에게 추천도를 공유하면 좋겠다는 아이디어를 가지고 만들었습니다. 또한 각 식물 데이터의 탄소 흡수량 데이터를 활용하여 개인이 보유한 식물 숫자를 기반하여 환경기여도를 확인 할 수 있게 추가적으로 구현하였습니다.

---

## 4. 이 프로젝트가 무엇을 하는건지?

**주사용자**  
친환경 식물 재배 난이도 및 탄소포집 성능 지표가 필요한 사용자

**특징**
- 개발자 친환경 원예 블로그 운영
- 소규모 ~ 대규모 모임 기능 지원
- 탄소포집 식물 리뷰

---

## 5. 아키텍처 구조

<!-- 클라우드크래프트 이미지 삽입 -->

**ERD**  
[ERD 바로가기]([URL](https://www.erdcloud.com/d/7gtfTPgReKwTqd7Eg))

---

## 6. 주요기능

### MVP 및 주요 페이지별 시나리오

<!-- 각자 1개씩 작성 -->

#### 기능 1 (담당: 윤성현)
- 로그인 및 인증 개발
- 회원 관리 BE/FE 개발
- 센서 및 센서 연동 BE/FE 
- 게시판(일반 게시판, 공지사항) Backend 개발 및 Frontend API 연동 담당

#### 기능 2 (담당: )
- 

#### 기능 3 (담당: )
- 

#### 기능 4 (담당: )
- 

---

## 시작 가이드

### 설치 및 실행 방법

```bash
# 예시
git clone ...
cd ...
